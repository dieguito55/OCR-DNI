...
JNIEXPORT jlong JNICALL
Java_com_xiomi_xiomi_NativeOcr_nativeInit(
    JNIEnv *env, jclass thiz, jstring jDetModelPath, jstring jClsModelPath,
    jstring jRecModelPath, jstring jConfigPath, jstring jLabelPath,
    jint cpuThreadNum, jstring jCPUPowerMode) {
  std::string detModelPath = jstring_to_cpp_string(env, jDetModelPath);
...
Java_com_xiomi_xiomi_NativeOcr_nativeProcessImage(
    JNIEnv *env, jclass thiz, jlong ctx, jstring jImagePath,
    jstring jVisualizedPath) {
  if (ctx == 0) {
    return cpp_string_to_jstring(
        env,
        "{\"text\":\"\",\"lines\":[],\"boxes\":0,\"error\":\"OCR no inicializado\"}");
  }
  std::string imagePath = jstring_to_cpp_string(env, jImagePath);
  std::string visualizedPath = jstring_to_cpp_string(env, jVisualizedPath);
  Pipeline *pipeline = reinterpret_cast<Pipeline *>(ctx);
  return cpp_string_to_jstring(env,
                               pipeline->ProcessImage(imagePath, visualizedPath));
}

JNIEXPORT jstring JNICALL
Java_com_xiomi_xiomi_NativeOcr_nativePrepareImageVariants(
    JNIEnv *env, jclass thiz, jlong ctx, jstring jImagePath,
    jstring jOutputDir) {
  if (ctx == 0) {
    return cpp_string_to_jstring(
        env,
        "{\"variants\":[],\"error\":\"OCR no inicializado\"}");
  }
  std::string imagePath = jstring_to_cpp_string(env, jImagePath);
  std::string outputDir = jstring_to_cpp_string(env, jOutputDir);
  Pipeline *pipeline = reinterpret_cast<Pipeline *>(ctx);
  return cpp_string_to_jstring(
      env, pipeline->ExportPreprocessVariants(imagePath, outputDir));
}

JNIEXPORT jboolean JNICALL
Java_com_xiomi_xiomi_NativeOcr_nativeRelease(JNIEnv *env, jclass thiz,
                                             jlong ctx) {
  if (ctx == 0) {
    return JNI_FALSE;
  }
