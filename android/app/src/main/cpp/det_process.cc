...
DetPredictor::Postprocess(const cv::Mat srcimg,
  ...
  if (shape_out.size() < 3 || outptr == nullptr) {
    return {};
  }
  ...
  int out_w = 0;
  if (shape_out.size() >= 4) {
    out_h = static_cast<int>(shape_out[shape_out.size() - 2]);
    out_w = static_cast<int>(shape_out[shape_out.size() - 1]);
  } else {
    out_h = static_cast<int>(shape_out[1]);
    out_w = static_cast<int>(shape_out[2]);
  }
  if (out_h <= 0 || out_w <= 0) {
    return {};
  }
  ...
  cv::Mat cbuf_map(out_h, out_w, CV_8UC1, cbuf.data());
  cv::Mat pred_map(out_h, out_w, CV_32F, pred.data());
